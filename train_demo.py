...
def main():
    ...
    parser.add_argument('--model', default='same',
            help='model name')
    ...
    elif model_name == 'same':
        model = Same(sentence_encoder, hidden_size=opt.hidden_size)
    ...
    for i in range(total_test_round):
        with torch.no_grad():
            cur_acc = framework.eval(model, 4, N, K, Q, 500, na_rate=opt.na_rate,
                ckpt=test_ckpt)
